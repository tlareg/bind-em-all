function bindSingle(context, methodName) {
  ...
  if (typeof context[methodName] !== 'function') {
    throw new Error(`bindEmAll expect "${methodName}" to be method on given ` +
      `context (context constructor name: "${context.constructor.name}")`)
  }
...
export default function bindEmAll(context, methods) {
  ...
  const methodsArr = (methodsArgType === 'string')
    ? methods.split(/\s+/).map(n => n.trim()).filter(n => !!n)
    : methods
